...
/**
 * razorpay enrolment plugin tests.
 *
...
class enrol_razorpay_testcase extends advanced_testcase {

    protected function enable_plugin() {
        $enabled = enrol_get_plugins(true);
        $enabled['razorpay'] = true;
    ...
    protected function disable_plugin() {
        $enabled = enrol_get_plugins(true);
        unset($enabled['razorpay']);
        $enabled = array_keys($enabled);
        set_config('enrol_plugins_enabled', implode(',', $enabled));
    }

    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('razorpay'));
        $plugin = enrol_get_plugin('razorpay');
        $this->assertInstanceOf('enrol_razorpay_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_razorpay', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $razorpayplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_razorpay_plugin $razorpayplugin  */
        $razorpayplugin = enrol_get_plugin('razorpay');
        ...
        $razorpayplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $razorpayplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $razorpayplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $razorpayplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $razorpayplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now-60*60);
        $razorpayplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now+60*60);

        $razorpayplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $razorpayplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $razorpayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $razorpayplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));


        $razorpayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $razorpayplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $razorpayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
    ...
    public function test_get_user_enrolment_actions() {
        ...
        // Only enable the razorpay enrol plugin.
        ...
        // razorpay enrolment has 2 enrol actions for active users when logged in as admin: edit and unenrol.
        ...
        $actions = $plugin->get_user_enrolment_actions($manager, $ue);
        // Teachers don't have the enrol/razorpay:unenrol capability by default, but have enrol/razorpay:manage.

 ...
 * @return string
 */
function theme_hurix_get_extra_scss($theme) {
    $scss = $theme->settings->scss;

    $scss .= theme_hurix_set_headerimg($theme);

    $scss .= theme_hurix_set_topfooterimg($theme);

    $scss .= theme_hurix_set_loginbgimg($theme);
 ...
 * @return string
 */
function theme_hurix_set_headerimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_hurix_set_topfooterimg($theme) {
    global $OUTPUT;
    $headercss='';
 ...
 * @return string
 */
function theme_hurix_set_loginbgimg($theme) {
    global $OUTPUT;
    ...
    $headercss = "#page-login-index.hurix-login #page-wrapper #page {background-image: url('$loginbgimg');}";
 ...
 * @return string
 */
function theme_hurix_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_hurix', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_hurix and not theme_boost (see the line above).
    ...
    // hurix scss.
    $hurixvariables = file_get_contents($CFG->dirroot . '/theme/hurix/scss/hurix/_variables.scss');
    $hurix = file_get_contents($CFG->dirroot . '/theme/hurix/scss/hurix.scss');

    // Combine them together.
    $allscss = $hurixvariables . "\n" . $scss . "\n" . $hurix;
 ...
 * @return string
 */
function theme_hurix_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 * @return mixed
 */
function theme_hurix_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $theme = theme_config::load('hurix');
 ...
 * @return string
 */
function theme_hurix_get_setting($setting, $format = false) {
    $theme = theme_config::load('hurix');
...
/**
 * Extend the hurix navigation
 *
 * @param flat_navigation $flatnav
 */
function theme_hurix_extend_flat_navigation(\flat_navigation $flatnav) {
    theme_hurix_add_certificatesmenuitem($flatnav);

    theme_hurix_delete_menuitems($flatnav);

    theme_hurix_add_coursesections_to_navigation($flatnav);
}
 ...
 *
 */
function theme_hurix_add_certificatesmenuitem(\flat_navigation $flatnav) {
    global $COURSE;

    try {
        if (!theme_hurix_has_certificates_plugin()) {
            return;
        }

        $actionurl = new \moodle_url('/theme/hurix/certificates.php');

        // Course page.
        if ($COURSE->id > 1) {
            $parentitem = $flatnav->find('competencies', \navigation_node::TYPE_SETTING);

            $actionurl = new \moodle_url('/theme/hurix/certificates.php', ['id' => $COURSE->id]);
        }
        ...
        if (!is_null($parentitem->parent)) {
            $certificatesitemoptions = [
                'action' => $actionurl,
                'text' => get_string('certificates', 'theme_hurix'),
                'shorttext' => get_string('certificates', 'theme_hurix'),
 ...
 * @param flat_navigation $flatnav
 */
function theme_hurix_delete_menuitems(\flat_navigation $flatnav) {
 ...
 * @param flat_navigation $flatnav
 */
function theme_hurix_add_coursesections_to_navigation(\flat_navigation $flatnav) {
    global $PAGE;
    ...
    if ($PAGE->course->format != 'singleactivity') {
        $coursesectionsoptions = [
            'text' => get_string('coursesections', 'theme_hurix'),
            'shorttext' => get_string('coursesections', 'theme_hurix'),
 ...
 * @return bool
 */
function theme_hurix_has_certificates_plugin() {

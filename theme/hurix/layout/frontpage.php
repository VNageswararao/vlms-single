...
/**
 * Frontpage layout for the hurix theme.
 *
 * @package   theme_hurix
...
if (isloggedin()) {
    ...
    echo $OUTPUT->render_from_template('theme_hurix/frontpage', $templatecontext);
} else {
    $sliderfrontpage = false;
    if ((theme_hurix_get_setting('sliderenabled', true) == true) && (theme_hurix_get_setting('sliderfrontpage', true) == true)) {
        $sliderfrontpage = true;
        $extraclasses[] = 'slideshow';
    }

    $numbersfrontpage = false;
    if (theme_hurix_get_setting('numbersfrontpage', true) == true) {
        $numbersfrontpage = true;
    }

    $sponsorsfrontpage = false;
    if (theme_hurix_get_setting('sponsorsfrontpage', true) == true) {
        $sponsorsfrontpage = true;
    }

    $clientsfrontpage = false;
    if (theme_hurix_get_setting('clientsfrontpage', true) == true) {
        $clientsfrontpage = true;
    }

    $bannerheading = '';
    if (!empty($PAGE->theme->settings->bannerheading)) {
        $bannerheading = theme_hurix_get_setting('bannerheading', true);
    }

    $bannercontent = '';
    if (!empty($PAGE->theme->settings->bannercontent)) {
        $bannercontent = theme_hurix_get_setting('bannercontent', true);
    }

    $shoulddisplaymarketing = false;
    if (theme_hurix_get_setting('displaymarketingbox', true) == true) {
        $shoulddisplaymarketing = true;
    }

    $disablefrontpageloginbox = false;
    if (theme_hurix_get_setting('disablefrontpageloginbox', true) == true) {
    ...
    echo $OUTPUT->render_from_template('theme_hurix/frontpage_guest', $templatecontext);
}

...
namespace theme_hurix\output\core;
...
use theme_hurix\util\extras;

/**
 * Renderers to align hurix's course elements to what is expect
 *
 * @package    theme_hurix
...
class course_renderer extends \core_course_renderer {
    ...
    protected function course_card_body(coursecat_helper $chelper, core_course_list_element $course, $coursenamelink) {
        ...
        $content .= html_writer::tag('h5', $coursenamelink, ['class' => 'card-title']);
    ...
    protected function course_card_footer(core_course_list_element $course) {
        ...
        if (isloggedin()) {
            ...
            $content .= html_writer::link(new moodle_url('/course/view.php', ['id' => $course->id]),
                get_string('access', 'theme_hurix'), ['class' => 'card-link btn btn-primary']);
        ...
        $content .= $this->course_custom_fields($course);
            ...
            $content .= html_writer::start_tag('div', ['class' => 'pull-right']);
            $content .= html_writer::link(new moodle_url('/enrol/index.php', ['id' => $course->id]),
                get_string('enroll', 'theme_hurix'), ['class' => 'card-link btn btn-primary']);
            $content .= html_writer::end_tag('div'); // End pull-right.

...
class certificates implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $certificates = new \theme_hurix\util\certificates($USER, $this->courseid);

        $issuedcertificates = $certificates->get_all_certificates();

        $title = get_string('certificatestitle', 'theme_hurix');
        $subtitle = get_string('subtitleallcertificates', 'theme_hurix');

        if ($this->courseid) {
            $subtitle = get_string('subtitlecoursecertificates', 'theme_hurix');
        }

...
namespace theme_porter\output\core;
...
use theme_porter\util\extras;

/**
 * Renderers to align porter's course elements to what is expect
 *
 * @package    theme_porter
    ...
    protected function course_card_footer(core_course_list_element $course) {
        ...
        if (isloggedin()) {
            ...
            $content .= html_writer::link(new moodle_url('/course/view.php', ['id' => $course->id]),
                get_string('access', 'theme_porter'), ['class' => 'card-link btn btn-primary']);

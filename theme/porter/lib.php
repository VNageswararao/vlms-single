 ...
 * @return string
 */
function theme_porter_get_extra_scss($theme) {
    $scss = $theme->settings->scss;

    $scss .= theme_porter_set_headerimg($theme);

    $scss .= theme_porter_set_topfooterimg($theme);

    $scss .= theme_porter_set_loginbgimg($theme);
 ...
 * @return string
 */
function theme_porter_set_headerimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_porter_set_topfooterimg($theme) {
    global $OUTPUT;
    $headercss='';
 ...
 * @return string
 */
function theme_porter_set_loginbgimg($theme) {
    global $OUTPUT;

    $loginbgimg1 = $theme->setting_file_url('loginbgimg', 'loginbgimg');
    $loginbgimg = $OUTPUT->image_url('login_bg', 'theme');


    $headercss = "#page-login-index.porter-login #page-wrapper #page {background-image: url('$loginbgimg');}";
    //$headercss.=".loginpanel .instructions{background-image: url('$loginbgimg1');}";
    return $headercss;
}
 ...
 * @return string
 */
function theme_porter_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_porter', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_porter and not theme_boost (see the line above).
    ...
    // porter scss.
    $portervariables = file_get_contents($CFG->dirroot . '/theme/porter/scss/porter/_variables.scss');
    $porter = file_get_contents($CFG->dirroot . '/theme/porter/scss/porter.scss');

    // Combine them together.
    $allscss = $portervariables . "\n" . $scss . "\n" . $porter;
 ...
 * @return string
 */
function theme_porter_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 * @return mixed
 */
function theme_porter_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $theme = theme_config::load('porter');
 ...
 * @return string
 */
function theme_porter_get_setting($setting, $format = false) {
    $theme = theme_config::load('porter');
...
/**
 * Extend the porter navigation
 *
 * @param flat_navigation $flatnav
 */
function theme_porter_extend_flat_navigation(\flat_navigation $flatnav) {
    theme_porter_add_certificatesmenuitem($flatnav);

    theme_porter_delete_menuitems($flatnav);

    theme_porter_add_coursesections_to_navigation($flatnav);
}
 ...
 *
 */
function theme_porter_add_certificatesmenuitem(\flat_navigation $flatnav) {
    global $COURSE;

    try {
        if (!theme_porter_has_certificates_plugin()) {
            return;
        }

        $actionurl = new \moodle_url('/theme/porter/certificates.php');

        // Course page.
        if ($COURSE->id > 1) {
            $parentitem = $flatnav->find('competencies', \navigation_node::TYPE_SETTING);

            $actionurl = new \moodle_url('/theme/porter/certificates.php', ['id' => $COURSE->id]);
        }
        ...
        if (!is_null($parentitem->parent)) {
            $certificatesitemoptions = [
                'action' => $actionurl,
                'text' => get_string('certificates', 'theme_porter'),
                'shorttext' => get_string('certificates', 'theme_porter'),
 ...
 * @param flat_navigation $flatnav
 */
function theme_porter_delete_menuitems(\flat_navigation $flatnav) {
 ...
 * @param flat_navigation $flatnav
 */
function theme_porter_add_coursesections_to_navigation(\flat_navigation $flatnav) {
    global $PAGE;
    ...
    if ($PAGE->course->format != 'singleactivity') {
        $coursesectionsoptions = [
            'text' => get_string('coursesections', 'theme_porter'),
            'shorttext' => get_string('coursesections', 'theme_porter'),
 ...
 * @return bool
 */
function theme_porter_has_certificates_plugin() {

...
class school {
     ...
     * @param object $reason The reason, including string and args.
     * @return bool Whether we were successful.
     */
    public function add_coins($userid, $coins, \core\event\base $event = null, lang_reason $reason = null) {
        global $CFG;
        ...
        // Include the reason.
        if ($reason) {
            $data['reason_lang'] = (object) [
                'string' => $reason->get_string(),
                'args' => $reason->get_args() ? $reason->get_args() : (object) []
            ];
        }
